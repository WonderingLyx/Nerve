...
class LoadTiffStack():
    ...
    def __call__(self, dicts):
        if self.keys == None:
            for k in dicts.keys():
                assert os.path.exists(dicts[k]), f"path invalid:{dicts[k]} "
                im = sitk.ReadImage(dicts[k])
                im = sitk.GetArrayFromImage(im)
                self.data[k] = im
                meta = {'orignal_shape': im.shape, 'mode':['L', 'L', 'L']}
                self.data[f'{k}_meta_dict'] = meta
        else:
            for k in self.keys:
                assert k in dicts.keys(), f"keys invalid {k}"
                im = sitk.ReadImage(dicts[k])
                im = sitk.GetArrayFromImage(im)
    ...
    def __init__(self, root_dir,is_train=True, is_test=False, is_MakePatches=True, transform_id=0):
        ...
        for r, dirs, images in os.walk(osp.join(self.data_dir, 'images')):
            self.images = map(lambda x: osp.join(r, x), filter(
                lambda x: x.endswith(('tif', 'tiff')), images))
            self.images = list(self.images)
            self.images.sort(key= lambda x: int(x.split('.')[0].split('-')[-1]))
            break

        for r, dirs, images in os.walk(osp.join(self.data_dir, 'labels')):
            self.seg = map(lambda x: osp.join(r, x), filter(
                lambda x: x.endswith(('tif', 'tiff')), images))
            self.seg = list(self.seg)
            self.seg.sort(key= lambda x: int(x.split('.')[0].split('-')[-1]))
            break
    ...
    def init_transforms(self):
        if self.transform_id == 0:
            self.train_transforms = Compose([
                LoadTiffStack(keys=["image", "label"]),
                ...
                RandCropByPosNegLabeld(
                    keys=["image", "label"], label_key="label", spatial_size=(128, 128, 128), pos=3, neg=1, num_samples=4
...
if __name__ == "__main__":
    ...
    {'height': tensor([150, 150]), spatial_shape: tensor([[150, 150],
        [150, 150]]), 'mode': ['L', 'L'], original_channel_dim: tensor([nan, nan], dtype=torch.float64), 'width': tensor([150, 150]), 'filename_or_obj': ['//root/shared-nvme/lyx/Project/Nerve/Data/Nerve/training/images/volume-013.tif', '//root/shared-nvme/lyx/Project/Nerve/Data/Nerve/training/images/volume-1412.tif'], affine: tensor([[[ 1.0000e+00,  0.0000e+00,  0.0000e+00,  0.0000e+00],
         [ 0.0000e+00,  1.0000e+00,  0.0000e+00,  0.0000e+00],
         [ 0.0000e+00,  0.0000e+00,  1.0000e+00,  0.0000e+00],
         [ 0.0000e+00,  0.0000e+00,  0.0000e+00,  1.0000e+00]],

        [[ 1.0000e+00, -1.2246e-16,  0.0000e+00,  0.0000e+00],
         [ 1.2246e-16,  1.0000e+00,  0.0000e+00,  0.0000e+00],
         [ 0.0000e+00,  0.0000e+00,  1.0000e+00,  0.0000e+00],
         [ 0.0000e+00,  0.0000e+00,  0.0000e+00,  1.0000e+00]]],
       dtype=torch.float64), 'format': ['TIFF', 'TIFF']}